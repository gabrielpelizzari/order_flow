...
CONSUMER_NAME = "stock-worker"


def decide_stock_status(simulation: str = "NONE") -> str:
    return "STOCK_FAILED" if simulation == "STOCK_UNAVAILABLE" else "STOCK_RESERVED"


def process_stock_event(event: dict[str, Any], channel: Any) -> None:
    ...
    target_status = decide_stock_status(simulation=event["data"].get("simulation", "NONE"))
    if order["status"] != target_status:
        apply_status_transition(order["status"], target_status)
        update_order_status(order_id, target_status)

    event_type = "stock.reserved" if target_status == "STOCK_RESERVED" else "stock.failed"
